...
fig.suptitle("Johnson UBVRI Fluxes for a "+str(wattage)+" W, "+str(Teff)+" K blackbody")
...
for band in Johnson:
	...
	flux = [luminosity/(4*np.pi*d**2) for d in distance_close]
	...
	ax[0,0].plot(distance_close*1e-6, flux, label=name)
	ax[0,0].set_title("Flux (W/m²)")
	ax[0,1].plot(distance_close*1e-6, fλ, label=name)
	ax[0,1].set_title("Fλ (W/m²/Å)")
	ax[1,0].plot(distance_close*1e-6, Jy, label=name)
	ax[1,0].set_title("Fν (Jy)")
	ax[1,1].plot(distance_close*1e-6, ABmag, label=name)
	ax[1,1].set_title("ABmag")
ax[0,0].legend()
ax[0,0].set_xlim(min(distance_close)*1e-6,max(distance_close)*1e-6)
...
fig.tight_layout()
#fig.savefig("Filters_Johnson_close.png", dpi=100)

fig, bx = plt.subplots(2, 2, sharex=True, sharey=False)
fig.suptitle("SDSS ugriz Fluxes for a "+str(wattage)+" W, "+str(Teff)+" K blackbody")
...
for band in SDSS:
	...
	Jy = [f/Δν * 1e26 for f in flux] # 1 Jy = 1e-26 W/m²/Hz
	ABmag = [8.9-2.5*np.log10(J) for J in Jy]
	bx[0,0].plot(distance_close*1e-6, flux, label=name)
	bx[0,0].set_title("Flux (W/m²)")
	bx[0,1].plot(distance_close*1e-6, fλ, label=name)
	bx[0,1].set_title("Fλ (W/m²/Å)")
	bx[1,0].plot(distance_close*1e-6, Jy, label=name)
	bx[1,0].set_title("Fν (Jy)")
	bx[1,1].plot(distance_close*1e-6, ABmag, label=name)
	bx[1,1].set_title("ABmag")
bx[0,0].legend()
bx[0,0].set_xlim(min(distance_close)*1e-6,max(distance_close)*1e-6)
...
fig.set_size_inches(8, 6.8)
fig.tight_layout()
#fig.savefig("Filters_Sloan_close.png", dpi=100)

fig, cx = plt.subplots(2, 2, sharex=True, sharey=False)
fig.suptitle("2MASS JHK Fluxes for a "+str(wattage)+" W, "+str(Teff)+" K blackbody")
...
for band in twoMASS:
	...
	Jy = [f/Δν * 1e26 for f in flux] # 1 Jy = 1e-26 W/m²/Hz
	ABmag = [8.9-2.5*np.log10(J) for J in Jy]
	cx[0,0].plot(distance_close*1e-6, flux, label=name)
	cx[0,0].set_title("Flux (W/m²)")
	cx[0,1].plot(distance_close*1e-6, fλ, label=name)
	cx[0,1].set_title("Fλ (W/m²/Å)")
	cx[1,0].plot(distance_close*1e-6, Jy, label=name)
	cx[1,0].set_title("Fν (Jy)")
	cx[1,1].plot(distance_close*1e-6, ABmag, label=name)
	cx[1,1].set_title("ABmag")
cx[0,0].legend()
cx[0,0].set_xlim(min(distance_close)*1e-6,max(distance_close)*1e-6)
...
fig.set_size_inches(8, 6.8)
fig.tight_layout()
#fig.savefig("Filters_2MASS_close.png", dpi=100)
